...
impl Keyboard {
    ...
    pub fn set_repeat_rate(&mut self, delay: u8, speed: u8) {
        ...
        let delay = delay.min(3);
        let speed = speed.min(31);
        let rate_byte = (delay << 5) | (speed & 0x1F);

        unsafe {
            // Wait until input buffer is clear
            while self.control_port.inb() & KeyboardStatus::INPUT_BUFFER_FULL.bits() != 0 {}
            self.data_port.outb(KeyboardCommand::SetSpeed as u8);
            // Wait for ACK
            while self.data_port.inb() != KeyboardResponse::Ack as u8 {}
            // Send rate byte
            while self.control_port.inb() & KeyboardStatus::INPUT_BUFFER_FULL.bits() != 0 {}
            self.data_port.outb(rate_byte);
            // Wait for ACK
            while self.data_port.inb() != KeyboardResponse::Ack as u8 {}
        }
    }
    ...
    fn set_led(&mut self, led: LedStatus, on: bool) {
        ...
        // Compose the LED byte: bit 0 = Scroll Lock, bit 1 = Num Lock, bit 2 = Caps Lock
        let mut led_byte = 0u8;
        if self.leds.contains(LedStatus::SCROLL_LOCK) { led_byte |= 0b001; }
        if self.leds.contains(LedStatus::NUM_LOCK)   { led_byte |= 0b010; }
        if self.leds.contains(LedStatus::CAPS_LOCK)  { led_byte |= 0b100; }

        unsafe {
            // Wait until input buffer is clear
            while self.control_port.inb() & KeyboardStatus::INPUT_BUFFER_FULL.bits() != 0 {}
            self.data_port.outb(KeyboardCommand::SetLed as u8);
            // Wait for ACK
            while self.data_port.inb() != KeyboardResponse::Ack as u8 {}
            // Send LED byte
            while self.control_port.inb() & KeyboardStatus::INPUT_BUFFER_FULL.bits() != 0 {}
            self.data_port.outb(led_byte);
            // Wait for ACK
            while self.data_port.inb() != KeyboardResponse::Ack as u8 {}
        }
    }
